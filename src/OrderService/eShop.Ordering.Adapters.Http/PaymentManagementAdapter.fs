...
let verifyPaymentMethod shouldAcceptPayment : PaymentManagementPort.VerifyPaymentMethod<HttpIoError> =
    fun unverifiedPayment ->
        if shouldAcceptPayment then
            UnverifiedPaymentMethod.verify
            >> PaymentManagementPort.VerificationResult.Success
        else
            PaymentManagementPort.VerificationResult.Failed
        <| unverifiedPayment
        |> TaskResult.ok
